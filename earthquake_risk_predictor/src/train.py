...
def train_model(model, X_train, X_test, y_train_class, y_test_class, y_train_reg, y_test_reg, save_path='model.pt'):
    ...
    for epoch in range(30):
        ...
        loss = loss_class + (2.0 * loss_reg)  # زيادة وزن الانحدار
        loss.backward()
        optimizer.step()
    ...
    with torch.no_grad():
        ...
        print(classification_report(y_test_class, preds_class_labels, target_names=['Hafif', 'Orta', 'Güçlü', 'Şiddetli', 'Felaket']))

        mse = mean_squared_error(y_test_reg, preds_reg)
        print("\n📉 Regression MSE:")
        print(mse)

        errors = np.abs(y_test_reg.values - preds_reg)
...
def predict_and_save_outputs(model, X_test, y_test_class, y_test_reg, output_path="data/predictions.csv"):
    global reg_scaler
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.eval()
...
def run_xgboost_regressor(X_train, X_test, y_train_reg, y_test_reg):
    ...
    for i, label in enumerate(["magnitude", "depth_km"]):
        ...
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(f"data/xgboost_results_{label}.png")

    pd.DataFrame(results).T.to_csv("data/xgboost_metrics.csv")
...
def plot_geographic_distribution(df, lat_col="latitude", lon_col="longitude", border_file="tr.json"):
    ...
    plt.tight_layout()
    os.makedirs("data", exist_ok=True)
    plt.savefig("data/geographic_distribution.png")
