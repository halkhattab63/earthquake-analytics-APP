...
# import folium
# from folium.plugins import MarkerCluster
# from folium import FeatureGroup, LayerControl, GeoJsonTooltip
# from shapely.strtree import STRtree
# import logging
...
# def assign_risk_to_roads(roads_gdf, quakes_df, buffer_dist=0.001):
#     logging.info("🚦 تصنيف الطرق حسب قربها من الزلازل...")
#     quake_points = gpd.GeoDataFrame(
#         geometry=gpd.points_from_xy(quakes_df.longitude, quakes_df.latitude),
#         crs="EPSG:4326"
#     )
...
#         matches_idx = quake_tree.query(geom.buffer(buffer_dist))
#         nearby_quakes = [
#             quake_geoms[i] for i in matches_idx
#             if quake_geoms[i].distance(geom) <= buffer_dist
#         ]
#         count = len(nearby_quakes)
...
#     quakes_fp="data/cleaned/earthquakes_cleaned.csv"
# ):
#     logging.info("🗺️ توليد خريطة تفاعلية...")
#     roads = gpd.read_file(roads_fp)
#     shelters = gpd.read_file(shelters_fp)
#     path = gpd.read_file(path_fp)
#     quakes = pd.read_csv(quakes_fp)
...
#         group = FeatureGroup(name=f"Road Risk: {level}")
#         subset = roads[roads["risk_level"] == level]
#         if "risk_level" in subset.columns and not subset.empty:
#             folium.GeoJson(
#                 subset,
#                 style_function=lambda x, col=color: {"color": col, "weight": 2},
#                 tooltip=GeoJsonTooltip(fields=["risk_level"], aliases=["Risk Level"])
#             ).add_to(group)
#         else:
#             folium.GeoJson(
#                 subset,
...
#         ).add_to(fmap)
#     else:
#         logging.warning("⚠️ Evacuation path is empty and was not added.")
...
# def generate_combined_paths_map(roads_fp, shelters_fp, path_files, output_html):
#     logging.info("🗺️ توليد خريطة متعددة المسارات...")
#     roads = gpd.read_file(roads_fp)
#     shelters = gpd.read_file(shelters_fp)
#     fmap = folium.Map(location=[shelters.geometry.y.mean(), shelters.geometry.x.mean()], zoom_start=13)
...
#     for i, path_fp in enumerate(path_files):
#         path = gpd.read_file(path_fp).to_crs("EPSG:4326")
#         folium.GeoJson(
#             path,
#             name=f"Evacuation Route {i+1}",
#             style_function=lambda x, color="green" if i == 0 else "blue": {"color": color, "weight": 4},
